...
class ShowAlert{
    ...
    class func showAlert(text: String,forState : AlertType,duration : TimeInterval = 4,userInteration : Bool = true){
        ...
        alertView.frame = CGRect(x: 10, y: ((UIScreen.main.bounds.height / 2) - alertView.bounds.height), width: UIScreen.main.bounds.width-20, height: 80)
        ...
        switch forState {
                    ...
                    case .success:
                        alertView.backgroundColor = .systemIndigo
                    case .unknown:
        ...
        UIView.animate(withDuration: 0.4, delay: 0, usingSpringWithDamping: 0.6, initialSpringVelocity: 0.8, options: .curveEaseIn) {
            ...
            Timer.scheduledTimer(withTimeInterval: 4, repeats: false) { time in
                ...
                UIView.animate(withDuration: 0.4) {
                    alertView.transform = CGAffineTransform.init(scaleX: 0, y: 0)
                }completion: { _ in
